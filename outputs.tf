output "vpc_id" {
  description = "VPC ID"
  value       = aws_vpc.abz_vpc.id
}

output "rds_endpoint" {
  description = "RDS public DNS"
  value       = aws_db_instance.abz_rds.endpoint
}

output "redis_endpoint" {
  description = "Redis ElastiCache public DNS"
  value       = aws_elasticache_cluster.abz_redis.cache_nodes[0].address
}

output "redis_port" {
  description = "Redis ElastiCache port"
  value       = aws_elasticache_cluster.abz_redis.cache_nodes[0].port
}

output "alb_dns" {
  description = "ALB public DNS"
  value       = aws_lb.abz_alb.dns_name
}
